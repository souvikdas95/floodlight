...
public abstract class ForwardingBase implements IOFMessageListener {
     ...
     * 
     * Push multicast routes to destined switches
     * 
     ...
     */
	public boolean pushRouteMF(MulticastPath route, Match match, OFPacketIn pi,
        ...
        /*
         *  Verify path and map inPort and outPorts per switch
         */
        ...
        for (int index = 0; index < switchPortList.size() - 1; index = index + 2) {
			...
			IOFSwitch inputSw =  switchService.getSwitch(inputSwId);
			IOFSwitch outputSw =  switchService.getSwitch(outputSwId);
            ...
            // Add to inPort map
        	swInPort.put(inputSw, inputPort);
        	
        	// Add to outPorts map
    		Set<OFPort> outPorts = swOutPorts.get(outputSw);
    		if (outPorts == null) {
     ...
     * the packet-in was also generated from the same switch. However, if the input
     * port of the packet-in is included in the outports, the function will still
     * push the packet-out.
     * 
